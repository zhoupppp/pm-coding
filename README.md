...
| 「帮我把数据库那个字段删掉」 | 直接删 | 先弹确认：影响范围、回滚方案、等你点头 |
...
| 产品研发 | 功能开发、Bug 修复 | 7 步：澄清→规格→架构→编码→测试→安全→上线 |
...
默认的 Claude 收到任务直接执行。这套配置会先判断类型、选流程、该澄清的澄清、该确认的确认——因为目标是「交付正确的产品」，不是「快速出代码」。
</details>

<details>
<summary><strong>我能只装部分功能吗？</strong></summary>
可以。规则文件是独立的，你可以删掉不需要的 rules/common/ 文件。setup-portable.sh 会告诉你缺了什么。
</details>
